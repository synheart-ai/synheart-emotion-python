...
def extract_14_features(rr_intervals_ms: List[float], mean_hr: Optional[float] = None) -> List[float]:
    ...
    if len(uniform_time) >= 16:
        ...
        freqs, psd = sp_signal.welch(interpolated_rr, fs=sampling_rate, nperseg=min(256, len(interpolated_rr)))
    ...
    if len(valid_arr) >= 3:
        ...
        if len(valid_arr) >= 16:
            ...
            dfa_alpha1 = float(np.polyfit(log_sizes, log_fluct, 1)[0]) if len(log_sizes) >= 2 else 0.0
        else:
            dfa_alpha1 = 0.0
    else:
    ...
    # Heart rate
    hr = float(mean_hr) if mean_hr is not None else (float((60000.0 / mean_rr) if mean_rr > 0 else 0.0))
    
    return [rmssd, mean_rr, sdnn, pnn50, hf, lf, hf_nu, lf_nu, lfhf, tp, sd1sd2, samp_en, dfa_alpha1, hr]
...
def _load_model_from_package(model_name: str):
...
class EmotionEngine:
    ...
    def __init__(
        ...
        try:
            ...
            self._session, self._input_names, self._class_names, self._metadata = _load_model_from_package(model_name)
    ...
    def consume_ready(self) -> List['EmotionResult']:
        ...
        with self._lock:
            ...
            if oldest_age < (self.config.window_seconds - 2.0):
                self._log("warn", f"Window insufficient: {oldest_age:.1f}s < {self.config.window_seconds}s")
                return results
            ...
            if len(all_rr) < self.config.min_rr_count:
                self._log("warn", f"Too few RR intervals: {len(all_rr)} < {self.config.min_rr_count}")
                return results
            ...
            feature_names = ['RMSSD', 'Mean_RR', 'HRV_SDNN', 'pNN50', 'HRV_HF', 'HRV_LF',
